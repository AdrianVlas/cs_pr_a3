...
void preMFTBigReadAction(void);//action до чтени€
void postMFTBigReadAction(void);//action после чтени€
...
void constructorMFTBigComponent(COMPONENT_OBJ *mftbigcomp)
{
  ...
  mftbigcomponent->preReadAction   = preMFTBigReadAction;//action до чтени€
  mftbigcomponent->postReadAction  = postMFTBigReadAction;//action после чтени€
...
void loadMFTBigActualData(void) {
 setMFTBigCountObject(); //записать к-во обектов
  //ActualData
   ...
   //MFT-IN 1 0 item
   value = arr[item].settings.param[TIMER_LOGIC_INPUT] & 0xffff;//LEDIN 0 —ƒ item
   tempReadArray[item*REGISTER_FOR_OBJ+2] = value;
   value = (arr[item].settings.param[TIMER_LOGIC_INPUT] >> 16) & 0x7fff;//LEDIN 1 —ƒ item
   tempReadArray[item*REGISTER_FOR_OBJ+3] = value;

   //Reset-I  0 item
   value = arr[item].settings.param[TIMER_RESET] & 0xffff;//LEDIN 0 —ƒ item
   tempReadArray[item*REGISTER_FOR_OBJ+4] = value;
   value = (arr[item].settings.param[TIMER_RESET] >> 16) & 0x7fff;//LEDIN 1 —ƒ item
...
//записать к-во обектов
  int cntObj = current_config.n_timer;   // ≥льк≥сть таймер≥в
  if(cntObj<0) return;
...
//action после чтени€
  if(mftbigcomponent->operativMarker[0]<0) return;//не было чтени€
}//
