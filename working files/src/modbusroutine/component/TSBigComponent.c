...
void preTSBigReadAction(void);//action до чтени€
void postTSBigReadAction(void);//action после чтени€
...
void constructorTSBigComponent(COMPONENT_OBJ *tsbigcomp)
{
  ...
  tsbigcomponent->preReadAction   = preTSBigReadAction;//action до чтени€
  tsbigcomponent->postReadAction  = postTSBigReadAction;//action после чтени€
...
void loadTSBigActualData(void) {
 setTSBigCountObject(); //записать к-во обектов
  ...
  //ActualData
   ...
   //In TC 0  item
   int value = arr[item].settings.param[TS_LOGIC_INPUT] & 0xffff;//LEDIN 0 —ƒ item
   tempReadArray[item*REGISTER_FOR_OBJ+0] = value;
   value = (arr[item].settings.param[TS_LOGIC_INPUT] >> 16) & 0x7fff;//LEDIN 1 —ƒ item
   tempReadArray[item*REGISTER_FOR_OBJ+1] = value;

   //Block TC 0  item
   value = arr[item].settings.param[TS_BLOCK] & 0xffff;//LEDIN 0 —ƒ item
   tempReadArray[item*REGISTER_FOR_OBJ+2] = value;
   value = (arr[item].settings.param[TS_BLOCK] >> 16) & 0x7fff;//LEDIN 1 —ƒ item
...
//записать к-во обектов
  int cntObj = current_config.n_ts;    // ≥льк≥сть блок≥в сигнал≥зац≥й
  if(cntObj<0) return;
...
//action после чтени€
  if(tsbigcomponent->operativMarker[0]<0) return;//не было чтени€
}//
