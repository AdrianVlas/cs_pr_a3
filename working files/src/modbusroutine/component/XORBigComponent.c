...
void postXORBigReadAction(void);//action после чтени€
...
void constructorXORBigComponent(COMPONENT_OBJ *xorbigcomp)
{
  ...
  xorbigcomponent->postReadAction  = postXORBigReadAction;//action после чтени€
...
//записать к-во обектов
  int cntObj = current_config.n_xor;  // ≥льк≥сть елемент≥в "¬икл.јЅќ"
  if(cntObj<0) return;
...
//action после чтени€
  if(xorbigcomponent->operativMarker[0]<0) return;//не было чтени€
}//
