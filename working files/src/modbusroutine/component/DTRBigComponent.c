...
void preDTRBigReadAction(void);//action до чтени€
void postDTRBigReadAction(void);//action после чтени€
...
void constructorDTRBigComponent(COMPONENT_OBJ *dtrbigcomp)
{
  ...
  dtrbigcomponent->preReadAction   = preDTRBigReadAction;//action до чтени€
  dtrbigcomponent->postReadAction  = postDTRBigReadAction;//action после чтени€
...
//action после чтени€
  if(dtrbigcomponent->operativMarker[0]<0) return;//не было чтени€
}//
