...
typedef struct
{
  ...
  uint32_t *max_row_p;                //вказівник на макчсимальну кількість рядків у даному пункті меню
  unsigned int max_row_c;             //константне значення макчсимальної кількості рядків у даному пункті меню
  
  void (*func_move)(void);            //Вказівник на функцію, для переміщення по вибраному меню  
  void (*func_edit)(unsigned int);    //Вказівник на функцію, для редагування вибраної позиції у вибраному меню
...
typedef struct
{
  ...
  uint32_t ctrl_analog_inputs;          //Ввімк./Вимк. Контролю приростів струмів
